...
func (m *VulnDefsMetadata) CloneVT() *VulnDefsMetadata {
	...
	r := &VulnDefsMetadata{}
	if rhs := m.LastUpdatedTime; rhs != nil {
		if vtpb, ok := interface{}(rhs).(interface{ CloneVT() *timestamppb.Timestamp }); ok {
			r.LastUpdatedTime = vtpb.CloneVT()
		} else {
			r.LastUpdatedTime = proto.Clone(rhs).(*timestamppb.Timestamp)
		}
	}
...
func (m *VulnDefsMetadata) CloneGenericVT() proto.Message {
	return m.CloneVT()
}
...
func (m *VulnDefsMetadata) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.LastUpdatedTime != nil {
		if vtmsg, ok := interface{}(m.LastUpdatedTime).(interface {
